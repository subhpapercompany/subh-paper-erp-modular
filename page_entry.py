...
def render():
    ...
    # ------------------ FORM 4: PRODUCTION ------------------
        ...
        # Book Weight is taken from Book Weight.xlsx col K when available.
        # Paper Consumption = Production Qty × Book Weight value.
        fetched_book_weight = fetch_book_weight_full(prod_code) if prod_code else None
        if prod_code and production_qty is not None:
            _computed_book_weight, _, calculated_board_size, board_consumption = fetch_production_tab5_calculations(
                conn, prod_code, production_qty
            )
            book_weight = float(fetched_book_weight) if fetched_book_weight is not None else float(_computed_book_weight)
            board_consumption = float(board_consumption)
            paper_consumption = float(production_qty) * float(book_weight)
        else:
